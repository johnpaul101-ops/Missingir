...
const CreateRoomModal = ({ showCreateRoomModal, setShowCreateRoomModal }) => {
  ...
  const handleCreateRoom = (e) => {
    ...
    const data = createRoom(roomName);
  ...
  return (
    <>
      <div
        className={`${showCreateRoomModal ? "block" : "hidden"} w-full h-screen bg-black/75 absolute left-0 top-0 z-40`}
        onClick={() => setShowCreateRoomModal(false)}
      ></div>
      <div
        className={`${showCreateRoomModal ? "block" : "hidden"} absolute z-50 top-96`}
      >
        <form
